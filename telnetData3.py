...
os_emp		=	0.0			#offset para calibracao
...
while True:
	if len(tn.read_some()) == 44:
		...
		fields = data.split(' ')
		for i in range(4):
			if (float(fields[i]) > 0 and len(fields[3]) == 11):			# necessita melhorar o comparador == 11
				ch[i] = (float(fields[i])-os)/(k_int*k*samples)-os_emp
				ch_treated[i] = ch[i]-ch[i]*0.0045
			else:
				ch_treated[i] = 0
